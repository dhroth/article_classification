...
def with_pdf (pdf_doc, fn, pdf_pwd, *args):
    ...
    try:
        ...
        #doc.initialize(pdf_pwd) #as of 2014/3/24: "PDFDocument.initialize() method is removed and no longer needed. A password is given as an argument of a PDFDocument constructor."
    ...
    for lt_obj in lt_objs:
        if isinstance(lt_obj, LTTextBox) or isinstance(lt_obj, LTTextLine):
            ...
            if images_folder is not None:
                ...
                #do nothing since no images folder was given
                pass
...
def _parse_first_page (doc, images_folder):
    """With an open PDFDocument object, get the pages and parse each one
    [this is a higher-order function to be passed to with_pdf()]"""
    rsrcmgr = PDFResourceManager()
    laparams = LAParams()
    ...
    text_content = []
    for i, page in enumerate(PDFPage.create_pages(doc)):
        if i != 0: #only look at the second page
            continue
        interpreter.process_page(page)
        # receive the LTPage object for this page
        layout = device.get_result()
...
def get_first_page (pdf_doc, pdf_pwd='', images_folder='/tmp'):
    """Process each of the pages in this pdf file and return a list of strings representing the text found in each page"""
    return with_pdf(pdf_doc, _parse_first_page, pdf_pwd, *tuple([images_folder]))
